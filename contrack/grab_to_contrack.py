"""Convert one raw GRAB sequence into a ConTrack reference clip (.h5), object + contact side only.

Everything comes from GRAB; the only operations are a fixed rigid transform into the ConTrack
world, cropping to a frame range, and re-indexing the contact labels. Nothing is re-optimised.

    ConTrack_world = Rm @ GRAB_world + tm            (Rm = +90 deg about z, verified on the shipped clips)
...
finger joints, 3 per finger in the order Index, Middle, Pinky, Ring, Thumb == ConTrack segments).
  is_contact[t,h,s] = any object vertex labelled with that segment at frame t
  points[t,h,s]     = mean position (object-local frame) of the vertices carrying that label,
                      NaN when there is no contact (ConTrack applies nan_to_num on load)
...
Example:
    python contrack/grab_to_contrack.py --grab-root /mnt/ssd1/shenghe/datasets/GRAB_dataset \
        --seq s1/teapot_pour_1 --out out/grab-s1_teapot_pour_1.h5
"""
...
URDF_NAMES = ("xarm7_xhand_left_hand.urdf", "xarm7_xhand_right_hand.urdf")
BASE_TRANSLATION = np.array([[0.0, 0.4, 0.0], [0.0, -0.4, 0.0]], np.float32)  # left, right
...
def main():
    ...
    with h5py.File(args.out, "w") as f:
        ...
        f.create_dataset("urdf_name", data=np.array([s.encode() for s in URDF_NAMES], dtype="S27"))
        f.create_dataset("qpos", data=np.full((T, 2, 19), np.nan, np.float32))
        ...
        c.create_dataset("contact_threshold", data=np.float32(contact["threshold"]))
